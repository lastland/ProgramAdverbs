...
(*
#[export] Polymorphic Instance Functor__beta@{i j k a} (F : Type@{i} -> Type@{j})
...
#[export] Polymorphic Instance Applicative__eta@{i j k a} (F : Type@{i} -> Type@{j})
...
Polymorphic Cumulative Record Alternative__Dict@{i j} (m : Type@{i} -> Type@{j}) := Alternative__Dict_Build {
  op_zgzpzl____ : forall {a : Type@{i}}, m a -> m a -> m a ;
  empty___ : forall {a : Type@{i}}, m a }.

Polymorphic Definition Alternative@{i j k} (f : Type@{i} -> Type@{j}) `{Applicative@{i j k} f} :=
  forall (r__ : Type@{k}), (Alternative__Dict f -> r__) -> r__.
Existing Class Alternative.

Polymorphic Definition op_zgzpzl__ `{g__0__ : Alternative f}
   : forall {a} , f a -> f a -> f a :=
  g__0__ _ (op_zgzpzl____ f).

Polymorphic Definition empty `{g__0__ : Alternative f} : forall {a}, f a :=
  g__0__ _ (empty___ f).

Notation "'_<|>_'" := (op_zgzpzl__).
...
#[export] Polymorphic Instance Monad__beta@{i j k a} (F : Type@{i} -> Type@{j})
...
#[export] Instance Eq_Int___ : Eq_ Int := fun _ k => k {|
                               op_zeze____ := fun x y => (x =? y)%Z;
                               op_zsze____ := fun x y => negb (x =? y)%Z;
                               |}.

#[export] Instance Ord_Int___ : Ord Int := fun _ k => k {|
...
#[export] Instance Eq_Integer___ : Eq_ Integer := fun _ k => k {|
                               op_zeze____ := fun x y => (x =? y)%Z;
                               op_zsze____ := fun x y => negb (x =? y)%Z;
                             |}.

#[export] Instance Ord_Integer___ : Ord Integer := fun _ k => k {|
  op_zl____   := fun x y => (x <? y)%Z;
  op_zlze____ := fun x y => (x <=? y)%Z;
  op_zg____   := fun x y => (y <? x)%Z;
  op_zgze____ := fun x y => (y <=? x)%Z;
...
#[export] Instance Eq_Word___ : Eq_ Word := fun _ k => k {|
                               op_zeze____ := fun x y => (x =? y)%N;
                               op_zsze____ := fun x y => negb (x =? y)%N;
                             |}.

#[export] Instance Ord_Word___ : Ord Word := fun _ k => k {|
...
#[export] Instance Eq_Char___ : Eq_ Char := fun _ k => k {|
                               op_zeze____ := fun x y => (x =? y)%N;
                               op_zsze____ := fun x y => negb (x =? y)%N;
                             |}.

#[export] Instance Ord_Char___ : Ord Char := fun _ k => k {|
  op_zl____   := fun x y => (x <? y)%N;
  op_zlze____ := fun x y => (x <=? y)%N;
  op_zg____   := fun x y => (y <? x)%N;
  op_zgze____ := fun x y => (y <=? x)%N;
...
#[export] Instance Eq_bool___ : Eq_ bool := fun _ k => k {|
                               op_zeze____ := eqb;
...
#[export] Instance Ord_bool___ : Ord bool := fun _ k => k {|
...
#[export] Instance Eq_unit___ : Eq_ unit := fun _ k => k {|
                               op_zeze____ := fun x y => true;
                               op_zsze____ := fun x y => false;
                             |}.

#[export] Instance Ord_unit___ : Ord unit := fun _ k => k {|
...
#[export] Instance Eq_comparison___ : Eq_ comparison := fun _ k => k
{|
...
#[export] Instance Ord_comparison___ : Ord comparison := ord_default compare_comparison.
...
#[export] Polymorphic Instance Eq_pair___ {a} {b} `{Eq_ a} `{Eq_ b} : Eq_ (a * b) := fun _ k => k
  {| op_zeze____ := eq_pair;
     op_zsze____ := fun x y => negb (eq_pair x y)
  |}.

#[export] Polymorphic Instance Ord_pair___ {a} {b} `{Ord a} `{Ord b} : Ord (a * b) :=
...
#[export] Polymorphic Instance Eq_list {a} `{Eq_ a} : Eq_ (list a) := fun _ k => k
  {| op_zeze____ := eqlist;
     op_zsze____ := fun x y => negb (eqlist x y)
  |}.

#[export] Polymorphic Instance Ord_list {a} `{Ord a}: Ord (list a) :=
...
Polymorphic Definition op_z2218U__ {b} {c} {a} : (b -> c) -> (a -> b) -> a -> c :=
  fun f g => fun x => f (g x).
...
#[export] Polymorphic Program Instance Eq___option {a} `{Eq_ a} : Eq_ (option a) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Ord__option {a} `{Ord a} : Ord (option a) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Eq___NonEmpty {a} `{Eq_ a} : Eq_ (NonEmpty a) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Functor__list@{j k} : Functor@{list.u0 j k} list :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Functor__option : Functor option :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Functor__pair_type {a} : Functor (GHC.Tuple.pair_type a) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Functor__arrow {r} : Functor (GHC.Prim.arrow r) :=
  fun _ k__ =>
...
#[export] Program Instance Applicative__list : Applicative list :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Applicative__option : Applicative option :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Applicative__arrow {a} : Applicative (GHC.Prim.arrow a) :=
  fun _ k__ =>
...
#[export] Program Instance Monad__list : Monad list :=
  fun _ k__ =>
...
#[export] Program Instance Functor__NonEmpty : Functor NonEmpty :=
  fun _ k__ =>
...
#[export] Program Instance Applicative__NonEmpty : Applicative NonEmpty :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Semigroup__option {a} `{Semigroup a} : Semigroup (option a) :=
...
#[export] Polymorphic Program Instance Semigroup__comparison : Semigroup comparison :=
...
#[export] Polymorphic Program Instance Semigroup__unit : Semigroup unit :=
...
#[export] Polymorphic Program Instance Semigroup__arrow {b} {a} `{Semigroup b} : Semigroup (a -> b) :=
...
#[export] Polymorphic Program Instance Semigroup__NonEmpty {a} : Semigroup (NonEmpty a) :=
...
#[export] Polymorphic Program Instance Semigroup__list {a} : Semigroup (list a) :=
...
#[export] Polymorphic Program Instance Applicative__pair_type {a} `{Monoid a}
...
#[export] Polymorphic Program Instance Monoid__option {a} `{Semigroup a} : Monoid (option a) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monoid__comparison : Monoid comparison :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monoid__unit : Monoid unit :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monoid__arrow {b} {a} `{Monoid b} : Monoid (a -> b) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monoid__list {a} : Monoid (list a) :=
  fun _ k__ =>
...
#[export] Program Instance Monad__NonEmpty : Monad NonEmpty :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monad__option : Monad option :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monad__arrow {r} : Monad (GHC.Prim.arrow r) :=
  fun _ k__ =>
...
#[export] Polymorphic Program Instance Monad__pair_type {a} `{Monoid a}
